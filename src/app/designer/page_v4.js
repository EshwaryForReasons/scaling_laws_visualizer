...
function KaplanFigure1Panel({ title, subtitle, data, xKey, xLabel, equation, xDomain, currentX }) {
  const xTicks = makeLogAxisTicks(xDomain, 6);
...
function KaplanMultiSeriesChart({
  ...
  const xTicks = makeLogAxisTicks(xDomain, 7);
    ...
    >
      <div className="flex h-full min-h-72 flex-col gap-2">
        <div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2">
          <div className="flex items-center justify-center text-[11px] text-zinc-500 [writing-mode:vertical-rl]">
            {ySpec.mathLabel}
          </div>
          <div
            className="grid gap-1 rounded-xl border border-zinc-800 bg-zinc-950/80 p-2"
            style={{
          ...
          >
            {reversedY.flatMap((y) =>
              xValues.map((x) => {
                ...
                const colorValue = logColor ? Math.log10(raw) : raw;
                const t = Number.isFinite(colorValue) ? (colorValue - minColor) / span : 0;

                return (
                  <div
                    key={`${valueKey}-${x}-${y}`}
                    className="aspect-square min-h-3 rounded-[0.35rem] border border-black/20 transition hover:scale-110 hover:border-white/60"
                    style={{ background: heatColor(t, lowerIsBetter) }}
                    title={`${xSpec.label}: ${human_num(x)}\n${ySpec.label}: ${human_num(y)}\n${valueLabel}: ${formatValue(raw)}\nN: ${human_num(row?.N)}\nD: ${human_num(row?.D)}\nRegime: ${row?.regime ?? "—"}`}
                  />
                );
              })
            )}
        ...
        <div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2 text-[11px] text-zinc-500">
          <div />
          <div className="flex items-center justify-between gap-2">
            <span>{human_num(xValues[0])}</span>
            <span className="text-zinc-400">{xSpec.mathLabel}</span>
            <span>{human_num(xValues[xValues.length - 1])}</span>
          </div>
        </div>
      </div>
    </ChartShell>
...
function ExpectedLossChart({ data, sweepSpec }) {
  ...
  const xDomain = isLog ? finitePositiveDomainFromData(data, "x", 1.02) : ["dataMin", "dataMax"];
  const xTicks = isLog ? makeLogAxisTicks(xDomain, 7) : undefined;
...
export default function ScalingLawDesignerPage() {
  ...
  const exportConfig = useMemo(() => {
    ...
    const secondRange = d_secondSweepSpec
      ? d_sweepRanges[d_effectiveSecondSweepVariable] ?? {
          min: d_secondSweepSpec.defaultMin,
          max: d_secondSweepSpec.defaultMax,
        }
      : null;

    const secondMinActual = d_secondSweepSpec && secondRange
      ? d_secondSweepSpec.scale === "log" ? logSliderToValue(secondRange.min) : secondRange.min
      : null;
    const secondMaxActual = d_secondSweepSpec && secondRange
      ? d_secondSweepSpec.scale === "log" ? logSliderToValue(secondRange.max) : secondRange.max
      : null;
    ...
    return {
      ...
      sweep: {
        ...
        y_min: d_is2DSweep ? secondMinActual : null,
        y_max: d_is2DSweep ? secondMaxActual : null,
