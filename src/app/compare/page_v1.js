...
function MultiCurveChart({
  ...
  const { data, series } = useMemo(
    () => makeChartData(curves, xMetric, yMetric),
    [curves, xMetric, yMetric]
  );

  const xDomain = logX ? finitePositiveDomain(curves, xMetric) : ["dataMin", "dataMax"];
  const yDomain = logY ? finitePositiveDomain(curves, yMetric) : finiteYDomain(curves, yMetric);
  const xTicks = logX ? makeLogAxisTicks(xDomain) : undefined;
  const yTicks = logY ? makeLogAxisTicks(yDomain) : undefined;
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 8, right: 18, bottom: 8, left: 8 }}>
          ...
          <YAxis
            ...
            stroke="#71717a"
            tick={{ fontSize: 12 }}
            tickFormatter={(y) => yMetric.includes("loss") ? Number(y).toFixed(3) : human_num(y)}
            label={{
              value: yLabel,
              angle: -90,
              position: "insideLeft",
          ...
          <BaseTooltip
            ...
            formatter={(v, name) => [
              yMetric.includes("loss") ? formatLoss(Number(v)) : human_num(Number(v)),
              name,
            ]}
          />
          <Legend wrapperStyle={{ fontSize: 11 }} />
          {Number.isFinite(referenceY) && (
            <ReferenceLine y={referenceY} stroke="#fbbf24" strokeDasharray="6 4" />
          )}
          {series.map((s) => (
            <Line
              key={s.key}
